...
`app.wasm` is generated by building the shared `web_counter` example in `build-web` when possible; if the local environment lacks a wasm toolchain, a minimal placeholder is emitted as a compatibility fallback.
