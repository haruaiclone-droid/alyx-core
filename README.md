...
`app.wasm` is generated by building the requested example in `build-web` (default: `web_counter`) when possible; if the local environment lacks a wasm toolchain, a minimal placeholder is emitted as a compatibility fallback.
